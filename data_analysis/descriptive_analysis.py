...
class descriptive_analysis(object):
    ...
    def normal_distribution(self, start=0, end=10, size=10):
        # 正态分布
        return np.random.normal(start, end, size=size)

    def uniform_distribution(self, start=0, end=10, size=10):
        # 均匀分布
        return np.random.randint(start, end, size=size)

    def data_characteristics(self, df, col_name):
    ...
    def z_score(self, df, col_name):

        ser = df[col_name]

        return df[col_name].apply(lambda x: (x - np.mean(ser)) / np.std(ser))

    def correlation(self, df, col_name1, col_name2):
...
if __name__ == '__main__':
    ...
    df.loc[0, "B"] = 3
    print(df)
    print(descriptive_analysis.data_characteristics(df, "B"))
    print(descriptive_analysis.correlation(df, "A", "B"))