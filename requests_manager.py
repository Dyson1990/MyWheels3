...
if __name__ == '__main__':
    ...
    for i in range(stock_list.shape[0]):
        try:
            ...
            df.columns = ['序号', '解除限售日期', '占总股本比例%', '数量（万股）','最新价（元）','当前市值（亿元）','限售股东一览']
            df['date'] = pd.to_datetime(df['解除限售日期']).copy()
            print(df)
            ...
            for date in df.index:
                s = s + date.strftime('%Y/%m/%d') + '：解禁' + str(df.loc[date, '占总股本比例%']) + '%\n'
            print(s)
            output = output.append({
                '股票代码':stock_list.index[i],
                '解禁期':s
            },
            ignore_index=True)
            output.to_excel('C:\\Users\\Administrator\\Desktop\\stock_解禁.xlsx')
    ...
    for i in range(stock_list.shape[0]):
        try:
            ...
            output = output.append({
                '股票代号':stock_list.index[i],
                '3月内机构评级统计': "买入：%s，增持：%s。" % (res[res['sjd'] == '3月内']['mr'].iloc[0], res[res['sjd'] == '3月内']['zc'].iloc[0]),
                '6月内机构评级统计': "买入：%s，增持：%s。" % (res[res['sjd'] == '6月内']['mr'].iloc[0], res[res['sjd'] == '6月内']['zc'].iloc[0]),
                '1年内机构评级统计': "买入：%s，增持：%s。" % (res[res['sjd'] == '1年内']['mr'].iloc[0], res[res['sjd'] == '1年内']['zc'].iloc[0])
            },
            ignore_index=True)
            print(output)
