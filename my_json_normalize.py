...
这本脚本几乎全部来自于json_normalize，可以使用pip install json_normalize来安装。
我这里针对json中的[]加了两句，作用是在递归过程中把[]转化为{}，key的只用[]中的序号代替。
"""
...
def _ckeck_dict(l0):
    if isinstance(l0, list):
        return {str(i0): obj0 for i0, obj0 in enumerate(l0)}
    else:
        return l0
